...
occur_all$duplicate <- "Duplicate" #41428 rows
occur_all$duplicate[first_match] <- "Unique" #8668 rows
...
centroids$long_round <- round(centroids$centroid_long, 2)
centroids$lat_round <- round(centroids$centroid_lat, 2)
  str(centroids)
# join occurrence points to centroid dataframe based on rounded lat and long
occur_centroid_join <- join(occur_counties, centroids, by = c("long_round", "lat_round"), type="left")#, match = "first")
  #str(occur_centroid_join)
  unique(occur_centroid_join$gps_determ)
  nrow(occur_centroid_join) #8551
  nrow(occur_centroid_join[which(is.na(occur_centroid_join$centroid_lat)),]) #8502
# mark records that may be centroids based on matching coordinates to centroid
occur_centroid_join$gps_determ <- as.character(occur_centroid_join$gps_determ)
for(i in 1:nrow(occur_centroid_join)){
  if(!is.na(occur_centroid_join$centroid_lat[i])){
    occur_centroid_join$gps_determ[i] <- "SC"
  }
}
unique(occur_centroid_join$gps_determ)
table(occur_centroid_join$gps_determ)
# order data to put county centroids / suspected county centroids last
occur_centroid_join <- occur_centroid_join[order(factor(occur_centroid_join$gps_determ,
                                  levels=c("G","L","SC","C"))),]
duplicates <- occur_centroid_join[duplicated(occur_centroid_join[c("speciesKey", "COUNTYNS")]),]
nrow(duplicates) #7355

to_remove <- duplicates[which(duplicates$gps_determ == "C" | duplicates$gps_determ == "SC"),]
nrow(to_remove) #641
to_remove$county_centroid_dup <- rep("x")
occur_dup_marked <- full_join(occur_centroid_join, to_remove)
